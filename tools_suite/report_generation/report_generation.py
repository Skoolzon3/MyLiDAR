...
from ..utils import format_global_encoding, format_point_format, gps_time_to_datetime, format_date, generate_pie_chart_from_counts, generate_return_bar_chart, generate_density_heatmap, log_step
...
class ReportGenerationTask(QgsTask):
    ...
    def run(self):
        try:
            log_step(self, self.tr("PROCESS START"), f"Input: {os.path.basename(self.input_filename)}, Report Format: {self.report_format}, Show Dock: {self.show_dock}")
            ...
            log_step(self, self.tr("STATISTICS EXTRACTED"), f"Classes: {len(unique_classes)}, Returns: {len(unique_returns)}", "info")
            ...
            if hasattr(las, "gps_time"):
                ...
                log_step(self, self.tr("TIME STATISTICS"), f"Min: {dt_min}, Max: {dt_max}, Total: {total_time}", "info")
            else:
            ...
            log_step(self, self.tr("BUILDING REPORT DATA"), "Compiling selected metadata and statistics into ReportData object", "info")
            data = ReportData(
            ...
            if self.show_dock:
                log_step(self, self.tr("GENERATING DOCK CONTENT"), "Creating text content and visualizations for the QGIS dock", "info")
                self.setProgress(60)
                self.report_text = generate_dock_content(self, data, self.tr)

                # --- Classification pie chart ---
                log_step(self, self.tr("GENERATING CLASSIFICATION PIE CHART"), "Creating pie chart for classification distribution if data is available", "info")
                ...
                log_step(self, self.tr("GENERATING RETURN NUMBER BAR CHART"), "Creating bar chart for return number distribution if data is available", "info")
                ...
                # --- Density Heatmap ---
                log_step(self, self.tr("GENERATING DENSITY HEATMAP"), "Creating density heatmap of point distribution if coordinates are available", "info")
            ...
            log_step(self, self.tr("SAVING REPORT"), f"Format: {self.report_format}, Path: {self.report_path}", "info")
            if self.report_format and self.report_path:
                if self.report_format == "pdf":
                    generate_pdf_report(self.parent, self.report_path, data, self.tr)
                    log_step(self, self.tr("PDF REPORT GENERATED"), f"Report saved to {self.report_path}", "info")
                elif self.report_format == "md":
                    generate_markdown_report(self.parent, self.report_path, data, self.tr)
                    log_step(self, self.tr("MARKDOWN REPORT GENERATED"), f"Report saved to {self.report_path}", "info")
                elif self.report_format == "txt":
                    generate_txt_report(self.parent, self.report_path, data, self.tr)
                    log_step(self, self.tr("TEXT REPORT GENERATED"), f"Report saved to {self.report_path}", "info")
                elif self.report_format == "tex":
                    generate_latex_report(self.parent, self.report_path, data, self.tr)
                    log_step(self, self.tr("LATEX REPORT GENERATED"), f"Report saved to {self.report_path}", "info")

            log_step(self, self.tr("PROCESS COMPLETE"), "Report generation finished successfully", "info")
    ...
    def finished(self, result):
        if result:
            if self.show_dock and getattr(self, "is_primary_task", True):
                ...
                log_step(self, self.tr("DOCK CREATED"), "QGIS dock created and added to the interface", "info")
            ...
            if self.is_zip_task and self.zip_output_path and self.temp_dir:
                try:
                    ...
                    log_step(self, self.tr("ZIP UPDATE"), f"{self.report_format.upper()} report added to ZIP at {self.zip_output_path}", "info")
                    QMessageBox.information(
                        self.parent.iface.mainWindow(),
                        self.tr("Success"),
                    ...
                    log_step(self, self.tr("ZIP ERROR"), f"{self.tr('Failed to add')} {self.report_format.upper()} {self.tr('to ZIP')}: {str(e)}", "critical")
